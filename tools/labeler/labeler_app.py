...
def start_server():
    ...
    with socketserver.TCPServer(("", PORT), LabelerHTTPRequestHandler) as httpd:
        try:
            httpd.serve_forever()
        except Exception as e:
...
if __name__ == '__main__':
    # Ensure current directory matches this script
    script_dir = os.path.dirname(os.path.abspath(__file__))
    os.chdir(os.path.join(script_dir, '..', '..', 'public'))
    
    # Start Python HTTP server in background thread
    server_thread = threading.Thread(target=start_server, daemon=True)
    server_thread.start()
    
    print(f"[INFO] Backend server running at http://localhost:{PORT}")
    time.sleep(0.5)
    
    # Try importing PySide6 / PyQt5 to open a native window
    USE_QT = False
    try:
    ...
    if USE_QT:
        print(f"[INFO] Launching native {USE_QT} Desktop Application...")
        app = QApplication(sys.argv)
        
        window = QMainWindow()
        window.setWindowTitle("Turbojet CAD 3D Bounding Box Labeler")
        window.resize(1280, 720)
        
        view = QWebEngineView()
        # Enable CORS and Local files
        view.settings().setAttribute(view.settings().WebAttribute.LocalContentCanAccessRemoteUrls, True)
        view.settings().setAttribute(view.settings().WebAttribute.LocalContentCanAccessFileUrls, True)
        
        view.setUrl(QUrl(f"http://localhost:{PORT}/labeler_ui.html"))
        window.setCentralWidget(view)
        
        window.show()
        sys.exit(app.exec())
    else:
        print("[INFO] PySide6 or PyQt5 is not installed.")
        print("[INFO] Running in fallback mode: opening in default web browser...")
        
        webbrowser.open(f"http://localhost:{PORT}/labeler_ui.html")
        
        print("[INFO] Press Ctrl+C to terminate the labeling server.")
        try:
            while True:
                time.sleep(1)
